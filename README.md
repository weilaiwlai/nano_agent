...
🚀 智能数据分析与报告平台（多智能体协作 + 数据洞察 + 可视化报告 + 自动化工作流）

## 项目概览

NanoAgent 是一个基于 LangGraph 构建的智能数据分析与报告平台，专注于数据驱动的决策支持和自动化报告生成。平台集成了多智能体协作、数据可视化、报告生成等核心能力，为企业提供端到端的数据分析解决方案。项目强调以下核心特性：

- **📊 智能数据分析**：专业的数据科学家智能体，支持复杂数据查询、统计分析和趋势预测
- **📋 自动化报告生成**：智能报告生成器，自动创建结构化的数据分析报告和可视化图表
- **🤖 多智能体协作**：Supervisor 智能统筹，DataScientist/Reporter/Assistant 专业分工协作
- **📈 数据可视化**：集成图表制作技能，支持多种图表类型和交互式可视化
- **🔧 数据工具层**：通过 MCP 协议提供数据库查询、文件处理等数据操作工具
- **🧠 智能记忆**：基于向量数据库的长期记忆，支持历史分析和上下文增强
- **⚡ 实时交互**：流式响应，实时展示分析进度和结果
